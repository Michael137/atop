...
run_tflite_benchmark( std::vector<std::string> const& model_paths,
                      ...
                      int processes = 1 );
std::future<shell_out_t>
run_tflite_app_benchmark( std::vector<std::string> const& model_paths,
                          std::map<std::string, std::string> const& options,
                          int processes = 1 );
std::future<shell_out_t>
...
class IoctlDmesgStreamer
{
   public:
	...
	ioctl_dmesg_t const& get_data() { return d_latest_data; }
	...
	float get_duration() const { return d_stream_latency; }

	DmesgProbes d_utilization_probe;
	bool d_is_data_fresh;

   private:
	double d_latest_ts;
	ioctl_dmesg_t d_latest_data;
	std::map<std::string, int> d_latest_interactions;
	std::vector<std::string> d_probes;

	// Latency of last "interactions" call
	float d_stream_latency;
};
...
class LogcatStreamer
{
   public:
	...
	logcat_out_t const& get_data() { return d_latest_data; }
	logcat_out_t more();

	bool d_is_data_fresh;

   private:
	std::string d_latest_ts;
	logcat_out_t d_latest_data;
	std::vector<std::string> d_probes;
	std::string d_logcat_tag_args;
};
...
void update_tflite_kernel_offload( atop::shell_out_t const& data,
                                   atop::BenchmarkStats& stats );
void update_tflite_kernel_gpu_offload( atop::shell_out_t const& data,
                                       atop::BenchmarkStats& stats );
void update_tflite_driver_offload( atop::shell_out_t const& data,
                                   atop::BenchmarkStats& stats );

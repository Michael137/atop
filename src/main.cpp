...
{
	auto output = atop::check_console_output(
	    fmt::format( "adb shell getprop {0}", prop ) );
	if( output.size() > 0 )
		return output[0];
	else
		return "";
...
static void adb_setprop( std::string_view prop, std::string prop_val )
{
	atop::check_console_output(
	    fmt::format( "adb shell setprop {0} {1}", prop, prop_val ) );
}

static void enable_dsp_log()
{
	atop::check_console_output(
	    R"(adb shell "echo 1 >> /sys/kernel/debug/adsprpc/global")" );
}
...
{
	atop::check_console_output(
	    R"(adb shell "echo 0 >> /sys/kernel/debug/adsprpc/global")" );
}
...
static void toggle_driver_logging()
{
	...
	LOG(
	    fmt::format( "Setting {0} from {1} to {2}", prop, prop_val, new_val ) );
...
static void ShowBenchmarkSummary( bool* popen, atop::BenchmarkStats& stats,
			...
			ImGui::TextUnformatted(
			    fmt::format( "{0}: {1} ms", p.first,
			                 static_cast<double>( p.second )
			                     / static_cast<double>( num_runs ) / 1.0e6 )
			        .c_str() );
		}
		else
			ImGui::TextUnformatted(
...
static void ShowIoctlBreakdown(
    std::map<std::string, std::map<std::string, int>>& breakdown, int num_runs )
{
		...
		for( auto&& cmd_p : p.second )
		{
			ImGui::TextUnformatted(
			    fmt::format( "\t{0}: {1}", cmd_p.first,
			                 static_cast<float>( cmd_p.second )
			                     / static_cast<float>( num_runs ) )
			        .c_str() );
		}
	}
...
int main( int argc, const char** argv )
{
	...
	struct
	{
	    ...
	    true, true,
	    true}; // All enabled since we call enable_kernel_logging() on startup

	static std::atomic<bool> exiting = false;

	std::string old_driver_logging_prop
	    = sim ? "" : adb_getprop( "debug.nn.vlog" );

	static bool driver_logging
	    = atop::util::string2bool( old_driver_logging_prop );
	...
	while( window.isOpen() )
	{
		...
		if( streamer.d_is_data_fresh && !data_got_consumed.ioctl )
		{
		...
		if( logcat_streamer.d_is_data_fresh && !data_got_consumed.logcat )
		{
			// TODO: use enum
		...
		ShowBenchmarkSummary(
		    &bench_summary_cb, bench_summary, num_runs,
		    frameworks[static_cast<size_t>( sel_framework )] );
		...
		bool tflite_hexagon_selected
		    = ( ( frameworks[static_cast<size_t>( sel_framework )]
		        ...
		        || ( frameworks[static_cast<size_t>( sel_framework )]
		             == atop::framework2string(
		                    atop::Frameworks::tflite_app ) ) )
		      && ( delegate_rb == 0 /* hexagon dsp delegate */
		           || delegate_rb == 4 /* qti-dsp */ );
			...
			switch( fr )
			{
				...
				case atop::Frameworks::tflite:
				{
					...
					std::map<std::string, std::string> options = {
					    ...
					    {"time_driver",
					     atop::util::bool2string( driver_logging )},
					...
					if( fr == atop::Frameworks::tflite_app )
						benchmark_futures_q.emplace(
						    atop::run_tflite_app_benchmark(
						        unzip_imgui_models( models ), options,
						        num_procs ) );
					else
		    ...
		    frameworks[static_cast<size_t>( sel_framework )] ) )
		{
			...
			case atop::Frameworks::tflite:
				...
				{
					atop::logger::verbose_info( fmt::format(
					    "Changed execution preference to: {0}",
					    exec_prefs[static_cast<size_t>( sel_exec_pref )] ) );
				}
				break;
			case atop::Frameworks::SNPE:
		...
		if( streamer.d_is_data_fresh && !data_got_consumed.ioctl )
		{
			auto data_to_log
			    = streamer
			          .get_data()[PROBE_IDX( streamer.d_utilization_probe )];
